...
母板不把十路现场线和所有交互器件挤在同一块板上：右侧接收端使用独立的窄长十路载板，左侧发射端使用独立电源子板（默认内置受保护 1S 电池，同时保留外接 10–30 V 端子），盖板上的按钮、OLED 预留、指示灯、蜂鸣器和扬声器使用 UI 子板。
...
UI 子板另留 0.96 英寸 I2C OLED、面板防护按钮、导光柱、带声学膜的扬声器/蜂鸣器，以及带硅胶帽的 USB-C 面板 bulkhead 参考位。
...
- `ui-panel-v0.2.kicad_pcb`：`58 × 28 mm` 盖面交互板，含 MX1.25 J7 对插、OLED、按钮、I2S 扬声器、蜂鸣器、指示灯和 USB-C bulkhead 参考口。
