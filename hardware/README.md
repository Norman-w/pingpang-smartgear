...
首版按 `1525 mm` 球台宽度、`25 mm` 台面厚度、`37 mm` 立柱外置距离和 `152.5 mm` 网顶高度起模；当前支架免打孔，固定 C 形夹体跨过台边，上夹板与桌面之间放置可替换 TPU/硅胶保护垫，固定下臂捕获一枚 M8 螺母；每侧夹紧螺杆下端由两枚预先对锁的 M8 螺母和打印旋钮刚性带动，圆头 M8×1.25 螺杆在台面下方顶起独立可动压块夹紧台底，不依赖 PETG 内螺纹或不明确的双固定螺纹；光学导轨位于台边与立柱内缘之间的开放空间，光轴覆盖球台边缘。最终兼容范围必须以实物测量、夹持力、滑移和网布张力记录为依据。当前不包含相机作为本体测量链路；外部视频设备只用于后续时间戳回放复核。原 `cad/net_post_x_clamp.scad` 是保留的外挂 X 夹具历史方案，不是当前主线。
