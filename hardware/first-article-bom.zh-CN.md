...
| 类别 | 数量 | 首样材料/说明 |
| --- | ---: | --- |
| PETG 结构件 | 浅黄色下段一体夹体/球头承座、台边外伸段下部 y 全深实心渐变支撑与非接触区实心桥体、网顶承载条、承托座、PVDF 座、旋钮、标定规、U 槽卡网圆柱 | PETG；按 manifest 中的 22 件导出矩阵打印；网顶承载条名义总宽 `1830 mm`，每侧网柱外边界距台边 `152.5 mm`；卡网圆柱左右各 1 根、实际 `Ø12 mm`，`Ø14 mm` 只作干涉校核；上下结构夹臂均为 `12 mm`，下部实心支撑靠台侧厚 `40 mm`、外侧收口为 `12 mm` 并形成斜底；M8 夹紧丝杆相对原包络加长 `12 mm`；深黄色上段和深灰色连接器不再作为零件 |
...
| M8 六角螺母 | 6 | 左右各 1 枚捕获在固定下臂；左右各 2 枚预先对锁后装入 18 齿圆角锯齿手拧旋钮，刚性带动螺杆 | 已由 CAD 固化 |
...
| 13 mm 采购球头 | 2 套 | 竖直姿态；商品上端固定螺纹为 1/4-20 外牙，下端默认 `13mm球【M8外牙】`，可选 1/4 内牙、1/4 外牙、3/8 外牙、M6/M10 外牙；上端沿 x 接入后盖隐藏的 1/4-20 捕获螺母，下端 M8 外牙 z− 直接拧入浅黄色下段的 M8 捕获螺母，提供偏航/俯仰/旋转微调 |
...
2. 检查浅黄色下段 M8 捕获螺母承座的方向、孔深和与网架/网布的净空；当前不装深黄色上段、`post_joint_sleeve` 或 `post_joint_key`。
...
4. 先打印并用真实器件试装 `m6_sensor_test_coupon`，确认锁紧螺母防转、头部方向、尾线让位和有效螺纹长度；再装左右两套 `m6_detector_body`、前后盖、底盖和竖直采购球头。每侧装入 10 个器件，按 `+10…+190 mm`、`20 mm` 节距布置，检查光轴、线缆弯曲、网夹净空和球路；确认商品上端 1/4-20 外牙进入后盖隐藏螺母、下端默认 M8 外牙进入浅黄色下段捕获螺母，再用最低/最高/中间通道完成偏航、俯仰、旋转微调并锁紧球头。
