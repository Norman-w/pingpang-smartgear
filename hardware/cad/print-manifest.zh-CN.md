...
1. 先用目标球台厚度样块（本版适用 `12…40 mm`，默认 `25 mm`），安装左右 `post_clamp_carrier`、固定上夹板胶皮、`clamp_pressure_pad`、`clamp_pressure_pad_guard`、`clamp_printed_screw`、`clamp_body_nut`、下臂上侧捕获螺母和旋钮内两枚 `clamp_knob_nut` 粗牙对锁螺母；确认不打孔、不碰台面、不干涉压块/螺杆工作区。背护罩四根定位柱插入压块盲孔后点胶，中心孔让 12 mm 杆身通过但挡住扁球头。粗牙件必须成套使用，不与 M8×1.25 金属螺母混配。
