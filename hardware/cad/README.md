...
本次网架连接约束：购买的竖直 13 mm 球头 z− 接口直接拧入浅黄色直立下段顶面的同轴 M8 承座；检测器/球头总成沿 x 移到网架立柱中心，取消横向黄色承托臂，不再从立柱侧面向上返；当前装配取消深黄色上段和深灰色独立连接器，承力路径回到浅黄色下段/桌下夹体，PETG 外壳只承担检测器自身保护与后盖 boss 的接口功能。

当前机械主线是 `net_stand.scad`：直接做出一套替换传统球网的内置式球网支架，网布和网顶 PVDF 振动监测预装在支架上，左右端通过传统的桌下 C 形夹体固定到球台。M8×1.25 螺杆完全位于台面下方，向上顶台底可动压块；螺杆不穿过球台，不要求也不允许给球桌打孔。光学主线是左右各一套十路 M6 直角对射 NPN 器件：每侧使用一根竖直 `10×56×216 mm` 可打印 PETG 长条主体，光学/M6 外丝轴沿 x 朝球台中心，右侧从 x+、左侧从 x- 外侧装入；蓝色护套/尾线局部沿 z-，整件绕光束 x 轴旋转 -45°，在外侧沿 x 穿入并向 y-/z- 让线；十个中心按 `20 mm` 节距排列，避开原来过密的器件干涉。主体用 x 向浅六角沉孔定位，M6 外丝直穿，朝台内的平滑面带一枚原配螺帽，固定螺丝不嵌入主体，线缆也不在主体里挖槽。下段立柱主体在网布通过处沿 x 切出 y 向 `3 mm` 过道，再在外侧开贯穿网高的 U 形槽；网布先穿过 3 mm 过道并放入 U 槽，再用两根独立打印的 PETG Ø12 mm 卡网圆柱沿 x 推入锁住；Ø14 mm 只作干涉校核，不是采购件。当前完整装配候选还包含 x- 光学端正球弧前盖、x+ 线缆端后盖（x- 接驳边直角、x+ 后部两个角圆角；x 背面中央、y=0、z 中心加厚 M8 boss）和底盖；采购 13 mm 球头保持竖直，横向 M8 外牙从各自 x 后端进入后盖 boss，球头 z- 竖直接口直接拧入浅黄色直立下段顶面的一体 M8 承座并与立柱中心同轴；检测器/球头总成沿 x 移到立柱中心，取消横向黄色承托臂；当前不再经过深黄色上段或深灰色独立连接器。壳体只保护、导向和提供接口，后续可改 CNC 或加金属嵌件。左右总成只保留实际需要的偏航、俯仰、旋转微调链，不再使用旧的梳齿条和水平三层云台方案。
...
- `assembly`：含球台截面、网布、网顶承载条、双侧浅黄色下段、桌下夹持、左右各十路 45° L 型 M6 直角器件、PETG 长条主体、完整 x 向分体前后盖、底盖、后盖 M8 boss、竖直采购球头、与立柱中心同轴的浅黄色下段顶面 M8 承座、PVDF 网顶安装座和十条名义光轴的装配预览；检测器/球头总成沿 x 移到立柱中心，取消横向黄色承托臂，深黄色上段和深灰色独立连接器不再显示；
...
- `post`：单侧浅黄色直立下段与立柱中心同轴的一体 M8 球头承座装配预览；`lower_stand_segment`：浅黄色下段、承座与固定 C 形夹一体打印，承座不再通过横向黄色承托臂连接；灰色夹体保留桌面夹持开口、台底压块和螺杆区域，桌边外侧非接触区沿 y 全深为实心桥体，上下结构夹臂均为 `12 mm`，外侧下部沿 y 全深为实心渐变支撑，靠台侧厚 `40 mm`、外侧收口为 `12 mm`；`post_segment`、`post_joint_sleeve` / `post_joint_key` 仅保留为旧版两段立柱的兼容诊断入口，不进入当前打印清单；`table_clamp`：单侧桌下夹持装配预览；`table_clamp_body`：固定 C 形夹体、实心桥体/渐变下部和下臂 M8 螺母捕获窝的几何检查件；`clamp_top_pad`：固定上夹板与桌面之间的可替换 TPU/硅胶保护垫；`clamp_pressure_pad`：台底可动压块/软垫占位；`clamp_screw`：带圆头且相对原包络加长 `12 mm` 的 M8×1.25 标准螺杆占位；`clamp_body_nut`：固定下臂中的标准 M8 螺母；`clamp_knob`：外径 `36 mm`、18 齿圆角锯齿外圈的手拧旋钮，带 M8 通孔和两枚对锁螺母捕获窝；`clamp_knob_nut`：旋钮内捕获的两枚标准 M8 对锁螺母；
...
- `m6_detector_shell_front` / `m6_detector_shell_rear` / `m6_detector_bottom_cover`：PETG x- 光学端正圆弧前盖、x+ 线缆端接驳边直角且仅自身后部两角圆滑的后盖（x 背面中央 y=0、z 中心加厚 M8 boss）和按组合轮廓制作的底盖；从 z+ 俯视时中间只表示前后盖分型边界、不建连线；安装/拆卸顺序为前盖从 z+、后盖从 z+、底盖从 z-，两盖共享主体 y± 连续边槽并分别占 x- / x+ 半段，沉头孔和底盖 Ø12 mm 线缆套管孔仍需真实器件首样复核；
- `m6_detector_net_connector`：旧版采购金属 90°连接器的兼容诊断包络，不属于当前装配，也不进入 PETG 打印清单；
- `m6_detector_mount`：当前默认显示主体、完整 x 分体壳体、底盖、后盖 M8 boss 和竖直采购球头的左右装配关系；检测器/球头总成沿 x 移到立柱中心，球头 z- 接口由浅黄色直立下段顶面的同轴 M8 承座承接，不显示横向黄色承托臂；`m6_ballhead` 是采购的 13 mm 球头外购件包络；完整 `assembly`/单侧支架还显示外侧 U 槽和独立 PETG 卡网圆柱；
...
首轮几何参数是球台宽度 `1525 mm`、网柱外边界距台边 `152.5 mm`、名义网架总宽 `1830 mm`、网顶高度 `152.5 mm`、10 个高度通道 `+10…+190 mm`（总成整体上抬 `20 mm` 后安装高度为 `+30…+210 mm`）、M6×0.75 直角对射器件和两侧加长的 `M8×1.25` 竖直夹紧螺杆。M6 中央主体按 `10×56×216 mm` 建模：水平光学/M6 外丝轴沿 x，蓝色尾线局部沿 z- 后绕 x 轴 -45°，主体中心在 y=0，右侧从 x+、左侧从 x- 外侧装入，单列中心距 `20 mm`；10 mm x 厚度通过 2 mm 头部卡入、5 mm 螺帽和约 1 mm 端部余量闭合，首件需复核公差和螺帽真实厚度。后盖 x 背面中央另设约 `14×18×36 mm` 加厚 boss，并开 Ø8.6 mm x 向通孔接采购 13 mm 球头的 M8 外牙；壳体当前已按 x 光学端/线缆端分段：x- 前盖保留正球弧，x+ 后盖接驳边保持直角、仅自身后部两角圆滑，底盖有统一线缆套管过孔；两盖共享 y± 连续边槽，槽舌、沉头孔和真实器件净空仍需首样复核，不视为最终打印放行。采购球头保持竖直，球头 z- 接口直接拧入浅黄色下段的一体 M8 承座，当前装配不再使用深黄色上段或深灰色独立连接器；偏航、俯仰、旋转微调仍由采购球头完成。依据 ITTF Technical Leaflet T2，当前自定义夹体最外点收敛为台边外 `160 mm`，仍满足项目下限 `130 mm`；它不是 ITTF 认证网夹，若复用商品夹，必须另测开口和台下投影。桌边外侧非接触区沿 y 全深做成实心桥体，上下结构夹臂均为 `12 mm`；外侧下部仍为沿 y 全深的 40→12 mm 实心渐变支撑，立柱外侧另设贯穿网高的 U 槽和 Ø12 mm PETG 卡网圆柱（Ø14 mm 干涉基准只用于首样配合检查），实际 PETG 层向、夹紧力、网布张力、球路净空和球头刚度仍需首样验证。球网、M6 光电器件、采购球头、PVDF 薄膜、线束、夹持软垫和金属标准件仍属于装配边界；OpenSCAD 结果不等同于最终强度、球台兼容性、电气输出语义或光学精度验收。
...
脚本把正式独立打印件写入被 Git 忽略的 `exports/net-stand-v0.1/`，同时生成 `manifest.json`（单位、来源 `PART`、左右侧、包围盒和封闭拓扑摘要）。输出目录若有旧版 STL，必须显式加 `--clean` 才会清理，避免历史外挂夹或旧光学载台混入当前首样；`test_export_net_stand_printables.py` 会固定当前 22 件 PETG/TPU 导出矩阵，并拒绝清单之外的 STL。M6 主体、前后盖、底盖、两根 Ø12 mm 卡网圆柱和带一体 M8 球头承座的浅黄色下段是当前 PETG 首样候选，采购球头与商品网夹不进入打印包；`assembly`、左右支架、`table_clamp`、`sensor_mount`、`m6_detector_mount` 等仍是组合装配预览，不进入这个通用打印包；输出 STL 仍需在切片软件中按实际材料、喷嘴和方向复核。
...
- `装配预览`：加载 22 个打印 STL 的真实装配坐标，同时显示球台、网布、左右各十路 45° L 型 M6 直角光电器件、PETG 长条主体、x 向分体前后壳、后盖 boss、底盖、竖直采购球头、浅黄色下段一体 M8 承座、左右外侧 U 槽和 Ø12 mm PETG 卡网圆柱、PVDF、M8 螺杆/螺母等非打印占位；不显示深黄色上段或深灰色独立连接器；壳体以半透明完整候选件显示；
...
`test_preview_consistency.py` 不需要 OpenSCAD，用于防止无 OpenSCAD 的轻量意图图继续沿用旧外挂夹具的关键参数；`validate_net_stand.py` 编译当前 `PART`、左右支架、桌板剖面夹持证据、浅黄色下段+一体承座+夹体、M6 45° L 型主体/斜向 7 字孔/后盖 boss/采购球头完整装配、圆头螺杆/固定下臂 M8 螺母/旋钮内两枚对锁螺母、参数探针、18/25/30 mm 台厚免打孔夹持矩阵和非法参数路径；参数探针还会在 Python 级别复核 M6 的 `20 mm` 节距、`+10…+190 mm` 高度、x 轴 -45° 姿态、后方浅六角定位、主体厚度、竖直球头 z- 接口、一体 M8 承座和下段支撑边界。`test_m6_machining_previews.py` 会实际导出主体左右两件局部首样/CNC 复用预览并检查封闭拓扑和关键包围盒；`test_m6_component_previews.py` 会实际导出当前主体、前后盖、底盖两侧共 8 个完整 STL，检查左右镜像、材料/打印候选元数据、封闭拓扑和主体 `10×56×216 mm` 包络。除组合装配预览外，成功导出的独立打印零件还会做 STL 封闭边拓扑检查。装配预览允许桌板、网布和电子/标准件包络重叠，只用于 PNG，不是打印件。后者从同一份参数源渲染当前装配、左右支架、桌下夹持、45° L 型 M6 光电器件阵列、竖直球头、浅黄色下段一体承座、PVDF 座和标定规，输出到 `rendered/net-stand-*.png`。CI 会把当前和历史两套渲染证据分别保存。
