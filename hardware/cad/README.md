...
- `post`：单侧浅黄色直立下段与立柱中心同轴的 M8 捕获螺母承座装配预览；`lower_stand_segment`：浅黄色下段、承座与固定 C 形夹一体打印，承座不再通过横向黄色承托臂连接；灰色夹体保留桌面夹持开口、台底压块和螺杆区域，桌边外侧非接触区沿 y 全深为实心桥体，上下结构夹臂均为 `12 mm`，外侧下部沿 y 全深为实心渐变支撑，靠台侧厚 `40 mm`、外侧收口为 `12 mm`；`post_segment`、`post_joint_sleeve` / `post_joint_key` 仅保留为旧版两段立柱的兼容诊断入口，不进入当前打印清单；`table_clamp`：单侧桌下夹持装配预览；`table_clamp_body`：固定 C 形夹体、实心桥体/渐变下部和下臂 M8 螺母捕获窝的几何检查件；`clamp_top_pad`：固定上夹板与桌面之间的可替换 TPU/硅胶保护垫；`clamp_pressure_pad`：台底可动压块/软垫占位；`clamp_screw`：带圆头且相对原包络加长 `12 mm` 的 M8×1.25 标准螺杆占位；`clamp_body_nut`：固定下臂中的标准 M8 螺母；`clamp_knob`：外径 `36 mm`、18 齿圆角锯齿外圈的手拧旋钮，带 M8 通孔和两枚对锁螺母捕获窝；`clamp_knob_nut`：旋钮内捕获的两枚标准 M8 对锁螺母；
...
脚本把正式独立打印件写入被 Git 忽略的 `exports/net-stand-v0.1/`，同时生成 `manifest.json`（单位、来源 `PART`、左右侧、包围盒和封闭拓扑摘要）。输出目录若有旧版 STL，必须显式加 `--clean` 才会清理，避免历史外挂夹或旧光学载台混入当前首样；`test_export_net_stand_printables.py` 会固定当前 22 件 PETG/TPU 导出矩阵，并拒绝清单之外的 STL。M6 主体、前后盖、底盖、两根 Ø12 mm 卡网圆柱和带 M8 捕获螺母承座的浅黄色下段是当前 PETG 首样候选，采购球头与商品网夹不进入打印包；`assembly`、左右支架、`table_clamp`、`sensor_mount`、`m6_detector_mount` 等仍是组合装配预览，不进入这个通用打印包；输出 STL 仍需在切片软件中按实际材料、喷嘴和方向复核。
...
当前 M6 总成的完整组件预览单独输出，不与正式 22 件 PETG/TPU 打印清单混用：
...
拼盘脚本只对已经导出的独立 STL 做 0/90° 旋转和平移，不缩放、不裁切、不把零件布尔合并。默认按 `256 × 256 × 256 mm` 打印床、边缘余量 `5 mm`、零件间距 `5 mm` 生成合并拼盘 STL；不同 `material_group` 严格分盘，当前源清单是 22 件，3 段约 `623.33 × 18 × 10 mm` 的网顶承载条和左右下段明确列为超尺寸件，新增两根 Ø12 mm 卡网圆柱可直接排入 PETG 拼盘，铝合金光学基座不进入拼盘，板数和已排版数量以最新 manifest 为准。
...
- `装配预览`：加载 22 个打印 STL 的真实装配坐标，同时显示球台、网布、左右各十路 45° L 型 M6 直角光电器件、PETG 长条主体、x 向分体前后壳、后盖 1/4-20 boss、底盖、竖直采购球头、浅黄色下段 M8 捕获螺母承座、左右外侧 U 槽和 Ø12 mm PETG 卡网圆柱、PVDF、M8 螺杆/螺母等非打印占位；不显示深黄色上段或深灰色独立连接器；壳体以半透明完整候选件显示；
