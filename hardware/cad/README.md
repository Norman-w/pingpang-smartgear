...
若需要直接检查实体几何外观，可运行 `python3 render_openscad_preview.py`；它从同一份 SCAD 生成双侧装配、左侧夹具、V 槽硬体、V 槽软垫、10 路光学模块包络、参考线端座、端座可打印本体和标定规八张 PNG，输出到 `rendered/`，与 `preview.py` 的意图图分开。CI 会把这些 OpenSCAD 渲染图作为 `smartgear-openscad-previews` artifact 保存。
