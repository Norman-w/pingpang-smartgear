...
前者编译当前 `PART`、左右支架、桌板剖面夹持证据、可打印光学导轨、光学镜像/单个载台、圆头螺杆/两枚 M8 螺母/旋钮捕获、参数探针、18/25/30 mm 台厚免打孔夹持矩阵和非法参数路径；除明确的 `assembly` 渲染预览外，成功导出的零件还会做 STL 封闭边拓扑检查。`assembly` 允许桌板、网布和装配包络重叠，只用于 PNG，不是打印件。后者从同一份参数源渲染当前装配、左右支架、桌下夹持及其桌板剖面、圆头螺杆、两个螺母、带捕获窝旋钮、可打印光学导轨、光学装配预览、单个载台、PVDF 座和标定规，输出到 `rendered/net-stand-*.png`。CI 会把当前和历史两套渲染证据分别保存。
