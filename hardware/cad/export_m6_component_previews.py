...
COMPONENTS = (
    ...
    ComponentSpec(
        ...
        "z+ 套入；x+ 线缆端圆角矩形朝外",
        "OpenSCAD 全局坐标；与主体装配基准一致",
        "后盖与前盖共用 y± 连续侧槽的 x+ 半段；x+ 背面中央在 y=0、z 中心适当增厚形成 PETG 支撑 boss，开 x 向 Ø7.0 1/4-20 通孔并内藏标准捕获螺母，供采购球头固定上端连接；左侧件按 x 镜像。采购球头保持竖直，选定的下端 M8 外牙只保留为独立光学支撑接口，尚未与固定网柱连接；固定网柱从黄灰交界 z=16 mm 共面起一体实心延伸至 z=372.5 mm，不进入 C 形座；网布/卡夹功能区仍只到 z=168.5 mm。检测器/球头总成沿 x 移到立柱中心仅表示空间对准，取消旧版横向承托臂，当前装配移除旧版独立上段外件和旧版独立连接器，首样需用实物复核 boss、独立支撑和球头的承力与耐久。",
    ),
    ComponentSpec(
