...
COMPONENTS = (
    ...
    ComponentSpec(
        ...
        "z+ 套入；x+ 线缆端圆角矩形朝外",
        "OpenSCAD 全局坐标；与主体装配基准一致",
        "后盖与前盖共用 y± 连续侧槽的 x+ 半段；x+ 背面中央在 y=0、z 中心适当增厚形成 PETG 支撑 boss，开 x 向 Ø8.6 M8 通孔供采购球头的水平 M8 外牙连接；左侧件按 x 镜像。采购球头保持竖直，z− 接口由采购金属 90°连接器从最低端承接，再以两枚 M6 贯穿螺栓直接连接网架立柱；首样需用实物固定件复核 boss 与连接器的承力和耐久。",
    ),
    ComponentSpec(
