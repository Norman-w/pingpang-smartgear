...
COMPONENTS = (
    ...
    ComponentSpec(
        ...
        "z+ 套入；x+ 线缆端圆角矩形朝外",
        "OpenSCAD 全局坐标；与主体装配基准一致",
        "后盖与前盖共用 y± 连续侧槽的 x+ 半段；x+ 背面中央在 y=0、z 中心适当增厚形成 PETG 支撑 boss，开 x 向 Ø8.6 M8 通孔供采购球头的水平 M8 外牙连接；左侧件按 x 镜像。采购球头保持竖直，z− 接口直接落在浅黄色直立下段顶面的同轴 M8 承座；检测器/球头总成沿 x 移到立柱中心，取消横向黄色承托臂，当前装配移除深黄色上段和深灰色独立连接器，首样需用实物复核 boss、承座和球头的承力与耐久。",
    ),
    ComponentSpec(
