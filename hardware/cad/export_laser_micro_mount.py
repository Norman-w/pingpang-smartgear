...
PARTS = [
    ...
    ("liner_lower", "球面衬垫下半", "TPU", "#46cfb2", True, False, [0,0,-5]),
    ...
    ("nuts", "调节螺母与锁紧螺母", "钢", "#b8c6cf", False, False, [0,0,20]),
    ("spring", "底部压缩弹簧", "弹簧钢", "#c4d2d9", False, False, [0,0,0]),
    ("cap_hardware", "夹座合拢螺钉与螺母", "钢", "#b8c6cf", False, False, [0,0,20]),
    ("retention", "激光头限位顶丝与螺母", "钢", "#b8c6cf", False, True, [-14,-10,9]),
    ...
    ("front_hardware", "前盖 M3×30 沉头螺钉与捕获螺母", "钢", "#b8c6cf", False, False, [-24,0,0]),
]
ARRAY_PARTS={"rail","front_cover","rear_cover","bottom_cover","bottom_gasket","front_hardware"}
...
def export(output_dir=OUTPUT, jobs=3):
    ...
    manifest = dict(schema_version="laser-micro-mount-0.1",units="mm",parameters=params,
