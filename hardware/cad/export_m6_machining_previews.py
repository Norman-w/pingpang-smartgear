...
purchased; its downward interface screws into the integrated light-yellow
lower-stand seat, so no separate connector STL is made.
"""
...
PREVIEW_PARTS = (
    MachiningPreviewSpec(
        ...
        "当前首样用 PETG 打印；后续可换 6061-T6 CNC。主体只有十个 20 mm 节距光学孔、浅 AF8 防转窝、盖件孔和 y± 边槽，不带 T 尾座、M8 接口或主体内线缆槽；后盖另有 PETG 加厚 1/4-20 boss，内藏标准捕获螺母，13 mm 采购球头保持竖直姿态，选定的下端 M8 外牙 z− 接口直接落在浅黄色直立下段顶面的同轴 M8 捕获螺母；检测器/球头总成沿 x 移到立柱中心，取消横向黄色承托臂，不使用深黄色上段或深灰色独立连接器。",
    ),
)
