...
const ASSEMBLY_STEPS = [
  { number: 1, label: "桌下夹紧与立柱", description: "两侧传统 C 形夹、保护垫、12 mm 大径/4 mm 螺距 PETG 粗牙螺杆和配套打印螺母固定在球台边缘；粗牙牙顶和凹槽各约 2 mm，牙根为连续实体带；上下结构舌头同步向台内延长 20 mm，台下有效伸入为 82 mm，粗牙压紧件位于下舌头中点；台底压紧盘放大为 Ø50。保留桌面夹持开口、压块和螺杆工作区，桌边外侧非接触区沿 y 全深做成实心桥体；上下结构夹臂均为 14 mm，靠球台侧下部支撑厚 40 mm，向外侧以 14 mm 下夹臂收口并形成斜底；底部手拧旋钮采用外径 40 mm、18 齿圆角锯齿握持圈，旋钮两侧 7 mm 加固斜墙与 C 夹本体一体打印，接触软垫仍独立可替换。" },
...
const PREVIEW_CACHE_BUSTER = "split-c-scheme-v10";
...
function makeReinforcedClampItems(entries) {
  ...
  for (const sideLabel of ["right", "left"]) {
    ...
    items.push(makeAssemblyItem({
      ...
      notes: "辅助包络按粗牙打印螺杆的 12 mm 大径、9.6 mm 芯径、4 mm 螺距和 2 mm 牙顶显示；上端仍顶住压块，旋钮位于螺杆末端。",
    }));
    items.push(makeAssemblyItem({
