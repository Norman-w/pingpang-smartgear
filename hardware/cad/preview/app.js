...
const PREVIEW_CACHE_BUSTER = "split-c-scheme-v6";
...
function assemblyVisible(item) {
  ...
  if (state.assembly.showSkpCandidate && sourcePart === "post_clamp_carrier") return false;
...
function renderMode() {
  ...
  if (designMode) {
    ...
    refs.modelCaption.textContent = state.uiMode === "exploded"
      ...
      : "主体与壳体总成预览；坐标约定为 x=光束左右、y=前后、z=竖直。球台、无网顶轨道的真实网布、PVDF、M6 45° L 型主体、x 向分体壳、后盖 boss、竖直采购球头和电子腔体按装配包络显示；完整装配态把每个打印件和外购件都放在同一套安装基准，爆炸偏移只在爆炸标签启用。灰色 C 形主体沿 y=0 分为操作者侧/对手侧两半，9 个横向 M5 连接点把两半锁成承力框；只有连接到电子仓空腔面的点增加 boss/十字肋，实心夹臂里的点只保留通孔/沉孔，避免外壳凸起。左下角连接孔向外侧移动，电子仓左下角再增加连接点；带绿色整体底座的整根黄色固定网柱与夹体分开打印；右侧桥体内形成约 117×40 mm 大电子仓，两半分开后从 y=0 分型面放入 PCB；主控板由底部柱支撑，电池放在主控板器件层上方的第二层托位，避开底部柱体，不再压穿电池；x+ 外侧墙和 boss 保留，上方约 15.5 mm 承力层、两侧 9 mm 承力壁、底部 4 mm 一体化斜底；绿色底座从 x+ 推入灰色让位腔，两个 Ø4/Ø4.4 孔负责锁紧，中央 Ø6×2 mm 底坑配 4 mm 钢球定位，黄色立柱从绿色顶面 z=16 mm 一体延伸到 z=260.5 mm，顶面中心开 M8 攻丝底孔，M6 球头下端 M8 直接进入固定网柱顶面中心孔，球头轴心与后盖 boss 共线；网布/卡夹仍只在 z=16…168.5 mm 的通道内工作，从绿色顶面起 30 mm 做 35×58→28×38 mm 的一体实心渐变。网布先穿过立柱 3 mm 过道，再从外侧开口装入全高 U 形卡网夹；盖板、boss、按钮/指示和线缆路径按当前机械包络检查；已移除旧版横向承托臂、独立滑轨和旧版独立上段外件/连接器。";
