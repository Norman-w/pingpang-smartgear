...
> 当前硬件已切换为用户指定的 M6 直角十路光电器件阵列 + 10×56×216 mm PETG 长条主体/后盖 x 背面中央加厚 1/4-20 boss + 竖直采购球头；球头下端 M8 只作为独立光学支撑待定义接口，不与固定网柱连接。本文提到的 10 路光栅位图和 GPIO 只是业务契约背景；在 M6 NPN 输出数量、电流、接口保护和最终引脚确认前，ESP32-S3/App 不得把机械十个通道位置当作已冻结的物理接口或有效高度数据。
