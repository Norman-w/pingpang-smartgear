...
| 实体几何可视证据 | `hardware/cad/render_openscad_preview.py` 从同一份 SCAD 直接渲染双侧装配、左侧夹具、光学模块包络、参考线端座和标定规；左侧夹具单件为 manifold/no-error，双侧装配、光学模块、端座和标定规均渲染成功，CI 将五张 PNG 作为独立 artifact 保存。 |
