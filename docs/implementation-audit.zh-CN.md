...
| 范围 | 当前证据 |
| --- | --- |
| X 型夹具 CAD | `hardware/cad/net_post_x_clamp.scad` 支持左右镜像、`SIDE=±1` 覆盖、10°…20° 运动参数、Ø8 轴孔、M8 螺杆/滚柱/压盖、方杆、10 路导轨和参考线端座；`validate_scad.py` 编译 9 个 PART、检查左右 STL 包围盒镜像、10°/20° 端点，并拒绝 25° 越界参数。 |
...
| 自动化入口 | `.github/workflows/validation.yml` 已配置主机/CTest/Schema/预览和 OpenSCAD 两个 job；本地 YAML 静态解析通过，推送后的 GitHub Actions 首次运行仍需以远端结果为准。 |
