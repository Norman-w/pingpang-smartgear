...
| M-03 | `validate_net_stand.py`；`render_net_stand_preview.py`；`render_m6_views.py`；装配页 | `152.5 mm` 网顶、`1830 mm` 名义网架总宽、左右各 `152.5 mm` 网柱外伸、当前夹体外轮廓名义外伸 `160 mm` 且结构下限为 `130 mm`、桌面夹持开口外的 y 全深实心桥体与渐变下部支撑（上下结构夹臂均 `12 mm`，靠台侧 `40 mm`、外侧 `12 mm`）、双侧替换式支架、3 段承载条与拼接片、左右各一根 10×56×216 mm M6 PETG 主体、后盖 x 背面中央加厚 1/4-20 boss 及 y± 实体桥接肋、前后盖/底盖、20 个 M6 直角器件包络、竖直采购球头、Ø24/Ø8.6 顶装 M8 直连承座、10 对名义光轴和 PVDF 安装位的参数与可视关系 | 真实网布张力、两侧外伸后的脚部/球路净空、实心桥体/渐变支撑耐久性、加长 M8 丝杆行程、左右主体平行度、网顶高度、M6 螺纹/螺母/光学中心、球头两端螺纹/捕获螺母和网夹接口、NPN 供电/电平和线缆净空；若宣称标准网夹兼容，还需量测 ITTF T2 的开口/台下投影边界 |
