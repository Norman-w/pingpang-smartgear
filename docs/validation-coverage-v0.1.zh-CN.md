...
| M-02 | `hardware/cad/validate_net_stand.py`；OpenSCAD CI 当前 `net_stand.scad` 各部件、夹体/压块/螺杆拆分、18/25/30 mm 台厚免打孔矩阵、镜像与非法参数路径 | 免打孔 C 形夹体跨过台边，M8 螺杆与可动压块的顶面都在台面底面以下，且左右导出可编译；旋钮现在有 M8 配合贯穿孔 | 实物样块/球台的夹紧力、台面损伤、立柱倾斜、螺杆行程和最终螺杆下端锁固方式 |
