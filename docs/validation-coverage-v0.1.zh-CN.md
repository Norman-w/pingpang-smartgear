...
| M-02 | `hardware/cad/validate_net_stand.py`；OpenSCAD CI 当前 `net_stand.scad` 各部件、四类 SIDE 镜像与非法参数路径 | 免打孔传统桌下上下夹持的参数关系、M8 螺杆位于台边外侧、夹持件跨过台边且左右导出可编译 | 实物样块/球台的夹紧力、台面损伤、立柱倾斜和螺杆行程 |
| M-03 | `validate_net_stand.py`；`preview.py`；`render_net_stand_preview.py` | `152.5 mm` 网顶、双侧支架、网顶承载条、10 mm 光栅档位、实际贯穿定位孔、全部十个参考高度和 PVDF/光学安装位的参数与可视关系 | 真实网布张力、左右立柱平行度、网顶高度和光学/PVDF 实物安装记录 |
