...
- 主机测试覆盖光栅位图/安静结束/超时/乱序保护、PVDF 双通道归并、完整与不完整波形特征/归档/非有限值保护、传感器健康快照质量门、聚合器对空校准 ID/越界健康位图和坏边界的 fail-closed 处理、GPIO 队列溢出、四种事件状态、端到端传感器归并、传输恢复、JSON Schema 和 CSV 轨迹回放。
