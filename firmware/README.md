...
- 主机测试覆盖光栅位图/安静结束/超时/乱序/反向时间边界保护、PVDF 双通道归并、完整与不完整波形特征/归档/非有限值保护、窗口外 DMA backlog 丢弃、比较器先到后由 DMA 完成波形并归并为 `touch_over` 的顺序路径、传感器健康快照质量门、健康快照在待决事件期间变化时的 fail-closed 处理、聚合器对空校准 ID/越界健康位图和坏边界的 fail-closed 处理、GPIO 队列溢出、四种事件状态、端到端传感器归并、传输恢复、JSON Schema 和 CSV 轨迹回放；ESP32 任务侧在队列溢出后会清空旧边沿，再等待新输入建立边界。
